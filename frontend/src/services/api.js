...
const API = axios.create({
  baseURL: BASE_URL,
  withCredentials: true, // necessário p/ Sanctum (se usar cookie)
...
const getCpf = () =>
  (typeof window === "undefined" ? null : localStorage.getItem("cpf"));
...
API.interceptors.request.use((cfg) => {
  ...
  if (c) cfg.headers["X-CPF"] = c.replace(/\D+/g, "").slice(0, 11);
...
// ----------------- CSRF (Sanctum) -----------------
const ROOT_URL = BASE_URL.replace(/\/api\/?$/, "/"); // .../api -> .../
const bare = axios.create({ baseURL: ROOT_URL, withCredentials: true });

export async function ensureCsrf() {
  try {
    await bare.get("sanctum/csrf-cookie");
  } catch {
    /* ignora em dev */
  }
}

// ----------------- Auth: setters/clearers -----------------
...
export function setCpfHeaderFromStorage() {
  const c = getCpf();
  if (c) API.defaults.headers.common["X-CPF"] = c.replace(/\D+/g, "").slice(0, 11);
...
export function setCpfHeader(cpf) {
  ...
  if (only) {
    localStorage.setItem("cpf", only); // opcional, mas ajuda o interceptor
    API.defaults.headers.common["X-CPF"] = only;
  } else {
